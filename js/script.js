...
function carregaElementosNaPagina(json) {
    ...
    for (let pessoa of json) {
        ...
        //IF PARA ADICIONAR A CLASSE ATIVO APENAS NO PRIMEIRO ITEM
        if (x == 0) {
            item.classList.add("ativo");
        }
        x++;
        item.onclick = function() {
            //PEGA AS INFORMAÇÕES E MUDA NA TELA DE INFORMAÇÕES
            ...
            document.querySelector(".ativo").classList.remove("ativo"); // REMOVE A CLASSE ATIVO DO ITEM
            this.classList.add("ativo"); // ADICIONA  A CLASSE ATIVO NO ITEM QUANDO CLICADO
        };
